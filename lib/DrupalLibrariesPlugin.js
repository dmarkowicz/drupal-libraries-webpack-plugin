const path = require('path'),
  YAML = require('yaml'),
  { mkdirp } = require("webpack/lib/util/fs"),
...
class DrupalLibrariesPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.normalModuleFactory.tap('DrupalLibrariesPlugin', nmf => {
      nmf.hooks.factorize.tapAsync("DrupalLibrariesPlugin",
        (data, callback) => {
          const result = this.opts.requirePattern.exec(data.request)
          if (result) {
            return callback(null, new DrupalLibraryModule(result[1]))
          }
          else {
            return callback();
          }
  ...
  async _writeResult(compiler, compilation, files) {
    ...
    files.forEach(file => {
      promises.push(new Promise(async (accept, reject) => {
        ...
        mkdirp(compiler.outputFileSystem, path.dirname(targetPath), err => {
          if (err) {
